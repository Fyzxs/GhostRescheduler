...
SingleEventDef.defineStandardProps({ // false = manually process
	...
	published_at: false, // alias for 'start'
	end: false,
	allDay: false
});
...
EventDateProfile.isStandardProp = function(propName) {
	return propName === 'start' || propName === 'date' || propName === 'end' || propName === 'allDay' || propName === 'published_at';
};
...
EventDateProfile.parse = function(rawProps, source) {
	var startInput = rawProps.start || rawProps.date || rawProps.published_at;
...
var DayGridEventRenderer = EventRenderer.extend({
	...
	fgSegHtml: function(seg, disableResizing) {
		...
		var titleText = (htmlEscape(eventDef.title || '') || '&nbsp;')
		titleHtml =
			'<span class="fc-title" title="' + titleText + '">' +
				titleText + // we always want one line of height
			'</span>';
